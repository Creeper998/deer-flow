...
**Terminal recursion-limit behavior preserves completed work.** A terminal
`GraphRecursionError` is recorded as `error` with
`stop_reason="turn_capped"` and published as an actionable `error` SSE event
without rolling back prior checkpoints or external side effects.

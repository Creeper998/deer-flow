...
- Routed models may emit bare citation markers such as `[citation:Title]` or
  `【citation:Title】` while placing the real links in a trailing `Sources` /
  `References` section. `core/citations/sources.ts::resolveBareCitationLinks()`
  repairs only unique title/site/domain matches from that explicit section
  before message or Markdown-artifact rendering. Keep the resolver
  provider-neutral, never invent a URL, and preserve ambiguous or code-fenced
  markers verbatim; the repaired Markdown feeds both citation badges and the
  existing deduplicated source/occurrence summary.
